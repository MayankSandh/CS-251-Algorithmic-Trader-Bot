...
std::cout << "+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+=+="<<std::endl;
std::cout << "ORDER NO: " << order1.orderno << std::endl;
std::cout << order1.stock_name << std::endl;
std::cout << "Validity: " <<order1.isValid << std::endl; //trade
std::cout << "Median Validity: "<< order1.medianvalid << std::endl; //run before arbitrage.
std::cout << "Time of Entry: "<< order1.t0 << std::endl;
std::cout << "Time to Expire: " << order1.expiry <<std::endl;
std::cout << "Quantity of Trade: " << order1.quantity << std::endl;
std::cout << "Price Per Quantity: "<<order1.price << std::endl;
std::cout << "STOCK MAP:-"<<std::endl;
int i = 0;
for (auto it = order1.stock_map.begin(); it!=order1.stock_map.end(); it++){
std::cout << " <"<< order1.stock_map[i].index << "> " << order1.stock_map[i].stock_name<< order1.stock_map[i].quantity<< std::endl;
i++;
}
std::cout << "Broker Name: " << order1.broker_name <<std::endl;
std::string action;
if (order1.action == 0){action = "BUY";}
else{action = "SELL";}
std::cout << "Action: " << action << std::endl;
std::cout << "Minimum Stock Index: " << order1.minindex <<std::endl;
std::cout << "Minimum Stock Name: " << order1.minstockname << std::endl;
std::cout << "--order info ends--" << std::endl << std::endl;
};
void printTradeList(){
...
void print_order(order order1)
{
	std::cout<<"price"<<order1.price<<std::endl;
	std::cout<<"quantity"<<order1.quantity<<std::endl;
	std::cout<<"expiry"<<order1.expiry<<std::endl;
	std::cout<<"broker"<<order1.broker_name<<std::endl;
	for( auto j : order1.stock_map)
	{
		std::cout<<"index of stock"<<j.index<<std::endl;
		std::cout<<"stock"<<j.stock_name<<std::endl;
		std::cout<<"stock quantity"<<j.quantity<<std::endl;
	}
}
...
void updateTradeList(std::vector<order> &trade_list, StocksTrie& stocks_trie, std::vector<std::string> new_lines, int & orderno, int & toEndTrader) {
   ...
   // std::cout<<new_lines[new_lines.size() - 2]<<std::endl;
...
void findcycles(cycle key, MaxHeap* heapwrite, std::vector <order> & trade_list, stock_level* current, StocksTrie & stocks_trie, int index)
{
	...
	{     
		...
		if( key.quantities[current->stock_index] == 0 ) 
		{
			//std::cout<<current->stock_index<<"--------"<<std::endl;
		//for(auto p : key.cycle_trades)
		//{
		//	std::cout<<p<<" ";
		//}
		//std::cout<<std::endl;
 (*heapwrite).insert(key); } 
 		return; 
	}	
...
// My code now assumes that 
	...
	while( current != NULL )
	{	
		//std::cout<<current->trades.size()<<std::endl;
		current->trades.clear();
		int stock_index = current->stock_index;
		//std::cout<<completemarker[stock_index].size()<<std::endl;
		//std::cout<<completemarker[stock_index].size()<<std::endl;
		...
		while((*heapwrite).heapSize != 0 ){ heapwrite->heap.pop_back(); heapwrite->heapSize -= 1; }
...
{	
    ...
    for(auto it : finalArbitrage.cycle_trades){
        if( it == 0 ) { i++; continue;}
	   	
	   if (trade_list[i].action == 0){
            bot_action = "SELL";
        ...
        std::string myTradeOffer = std::to_string(commonTimer.load()) + " " + trader_bot_name + " " + bot_action + " " + trade_list[i].stock_name + " $" + std::to_string(price) + " #" +  std::to_string(it) + " -1";
